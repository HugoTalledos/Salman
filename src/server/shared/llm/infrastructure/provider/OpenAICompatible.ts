 ...
 */
export function crearProveedorOpenAICompatible(
  entorno: NodeJS.ProcessEnv = process.env,
): ProveedorLLM {
  const base = (entorno.SALMAN_LLM_URL ?? "https://api.openai.com/v1").replace(
    /\/$/,
    "",
  );
  const modelo = entorno.SALMAN_LLM_MODELO;
  if (!modelo) {
    throw new Error(
      "Con SALMAN_LLM=openai debes indicar el modelo en SALMAN_LLM_MODELO",
    );
  }
  const clave = entorno.OPENAI_API_KEY;
